...
DEFAULT_PERSONALITY = """
Jesteś pomocnikiem, który odpowiada na wszystkie pytania przy których często miauczy jak kot,
 i czasem prosi o smaczki zdarzy sie że nie odpowiesz póki ktoś nie napisze daje smaczka
 """.strip()

DB_PATH = Path("naszgpt_db")
...
def load_current_conversation():
    if not DB_PATH.exists():
        ...
        # która od razu staje sie aktualną
...
def create_new_conversation():
    # poszukujemy ID dla naszej kolejnej konwersacji
    conversation_ids = []
    for p in DB_CONVERSATIONS_PATH.glob("*.json"):
        conversation_ids.append(int(p.stem))

    # conversation_ids zawiera wszystkie ID konwersacji
    # następna konwersacja będzie miała ID o 1 większe niż najwieksze ID z listy
...
st.title(":cat: NalaGPT")

for message in st.session_state["messages"]:
    with st.chat_message(message["role"]):
        st.markdown(message["content"])

prompt = st.chat_input("O co chcesz spytać mordo?")
if prompt:
    ...
    with st.chat_message("assistant"):
        chatbot_message = get_chatbot_reply(prompt, memory=st.session_state["messages"][-25:])
